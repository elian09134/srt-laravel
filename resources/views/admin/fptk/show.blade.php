    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2 mb-1">
                <a href="{{ route('admin.fptk.index') }}" class="text-slate-400 hover:text-indigo-600 transition-colors">
                    <i class="fas fa-arrow-left text-sm"></i>
                </a>
                <h1 class="text-xl font-semibold text-slate-800">Detail FPTK #{{ $fptk->id }}</h1>
            </div>
            <p class="text-sm text-slate-500 ml-7">Informasi lengkap permintaan tenaga kerja</p>
        </div>
        <div class="flex items-center space-x-3">
            @if($fptk->status === 'pending')
                <button disabled class="inline-flex items-center px-4 py-2 bg-gray-400 text-gray-200 font-medium rounded-lg shadow-md cursor-not-allowed opacity-60">
                    <svg class="w-5 h-5 mr-2 text-gray-200" fill="currentColor" viewBox="0 0 20 20">
